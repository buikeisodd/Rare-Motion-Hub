      ...
      <main className="mx-auto max-w-5xl py-8 sm:py-12">
        <section className="grid gap-7 sm:grid-cols-[9rem_1fr] sm:gap-10 lg:grid-cols-[12rem_1fr] lg:gap-14">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-6 flex flex-wrap gap-x-8 gap-y-3 text-sm"><span><strong className="text-base">{posts.length}</strong> posts</span><span><strong className="text-base">{profile.followerCount || 0}</strong> followers</span><span><strong className="text-base">{profile.followingCount || 0}</strong> following</span></div>
            <div className="mt-5 max-w-xl"><p className="font-semibold">{profile.name}</p><p className="mt-1 whitespace-pre-wrap text-sm text-secondary-label">{profile.bio || 'Music maker and creative.'}</p></div>
