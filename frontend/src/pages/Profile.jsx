...
import { ArrowLeft, Camera, Check, Edit3, Loader2, MessageCircle, MoreHorizontal, Play, UserPlus, X } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-primary-background px-4 pb-32 text-primary-label sm:px-8 md:pb-20">
      ...
      <main className="profile-dossier mx-auto w-full max-w-5xl py-5 sm:py-8">
        ...
        <section className="profile-dossier-hero grid gap-5 rounded-[1.5rem] border border-border bg-shading/35 p-4 shadow-sm sm:grid-cols-[10rem_1fr] sm:gap-7 sm:p-6 lg:grid-cols-[13rem_1fr] lg:gap-10">
          ...
          <div className="min-w-0">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-5"><div className="min-w-0"><h1 className="truncate text-2xl font-semibold sm:text-3xl">@{profile.username || profile.name || 'user'}</h1><p className="mt-1 truncate text-sm text-secondary-label">{profile.name || 'Profile'}</p></div><div className="flex flex-wrap gap-2">
              {profile.id === user?.id ? <div className="relative"><button type="button" onClick={() => setProfileMenuOpen((value) => !value)} className="grid h-11 w-11 place-items-center rounded-xl bg-shading transition-colors hover:bg-highlight" aria-label="Profile actions" aria-expanded={profileMenuOpen}><MoreHorizontal className="h-5 w-5" /></button>{profileMenuOpen && <div className="absolute right-0 top-full z-20 mt-2 w-56 rounded-2xl border border-border panel-bg p-1.5 shadow-2xl"><button type="button" onClick={() => { setProfileMenuOpen(false); setEditing(true); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold hover:bg-highlight"><Edit3 className="h-4 w-4" />Edit profile</button><button type="button" onClick={() => { setProfileMenuOpen(false); setAccountChooserOpen(true); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold hover:bg-highlight"><UserPlus className="h-4 w-4" />Previously logged in accounts</button><button type="button" onClick={() => { setProfileMenuOpen(false); navigate('/login'); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold hover:bg-highlight"><ArrowLeft className="h-4 w-4 rotate-180" />Log into another account</button></div>}</div> : <button onClick={toggleFollow} className={`inline-flex items-center justify-center gap-2 rounded-xl px-5 py-2.5 text-sm font-semibold transition-colors ${following ? 'bg-shading hover:bg-highlight' : 'bg-primary-label text-primary-background hover:opacity-85'}`}>{following ? <Check className="h-4 w-4" /> : <UserPlus className="h-4 w-4" />}{following ? 'Following' : profile.followsYou ? 'Follow Back' : 'Follow'}</button>}
            ...
            <div className="mt-6 grid grid-cols-3 gap-2 border-y border-border py-4 text-sm"><span><strong className="block text-lg">{posts.length}</strong><small className="text-secondary-label">previews</small></span><span><strong className="block text-lg">{profile.followerCount || 0}</strong><small className="text-secondary-label">followers</small></span><span><strong className="block text-lg">{profile.followingCount || 0}</strong><small className="text-secondary-label">following</small></span></div>
        ...
        <section className="mt-12 border-t border-border pt-6"><div className="mb-5 flex items-center gap-3 text-sm font-semibold uppercase tracking-[0.16em] text-primary-label"><span className="h-2 w-2 rounded-full bg-primary-label" />Published previews<span className="ml-auto text-xs font-normal tracking-normal text-secondary-label">{posts.length} total</span></div>{posts.length === 0 ? <p className="rounded-2xl border border-dashed border-border py-12 text-center text-sm text-secondary-label">No published previews yet.</p> : <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4">{posts.map((post) => <Link key={post.id} to={`/project/${post.projectId}`} className="group relative aspect-square overflow-hidden rounded-2xl bg-highlight shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">{post.coverArt && <img src={post.coverArt} alt="" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />}<div className="absolute inset-0 grid place-items-center bg-black/20 opacity-0 transition-opacity group-hover:opacity-100"><Play className="h-8 w-8 fill-white text-[#34483B]" /></div><div className="absolute bottom-2 left-2 right-2 truncate rounded-lg bg-[#F3EBDD]/85 px-2 py-1 text-xs font-semibold text-[#34483B] opacity-0 transition-opacity group-hover:opacity-100">{post.title}</div></Link>)}</div>}</section>
      </main>
      {accountChooserOpen && <div className="fixed inset-0 z-[85] grid place-items-center bg-black/60 p-4 backdrop-blur-xl" onClick={() => setAccountChooserOpen(false)}><div className="rmh-modal-panel w-full max-w-sm rounded-3xl border border-border p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}><div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-bold">Switch account</h2><button type="button" onClick={() => setAccountChooserOpen(false)} className="grid h-9 w-9 place-items-center rounded-xl bg-shading hover:bg-highlight" aria-label="Close"><X className="h-4 w-4" /></button></div><p className="mb-3 text-sm text-secondary-label">Choose an account already used on this device, or continue with a new one.</p><button type="button" onClick={() => navigate('/login')} className="flex w-full items-center gap-3 rounded-xl bg-shading p-3 text-left hover:bg-highlight"><UserPlus className="h-5 w-5" /><span><strong className="block">{user?.email || 'Current account'}</strong><small className="text-secondary-label">Existing account</small></span></button><button type="button" onClick={() => navigate('/login')} className="mt-2 flex w-full items-center gap-3 rounded-xl p-3 text-left hover:bg-highlight"><UserPlus className="h-5 w-5" /><span><strong className="block">Use another account</strong><small className="text-secondary-label">Sign in with different credentials</small></span></button><button type="button" onClick={() => navigate('/login?mode=register')} className="mt-2 flex w-full items-center gap-3 rounded-xl p-3 text-left hover:bg-highlight"><UserPlus className="h-5 w-5" /><span><strong className="block">Create a new account</strong><small className="text-secondary-label">Join Starlight Station</small></span></button></div></div>}
      {editing && profile.id === user?.id && <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/70 p-0 backdrop-blur-sm sm:items-center sm:p-4">
        <form onSubmit={saveProfile} className="rmh-modal-panel max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-3xl border border-border p-5 shadow-2xl sm:rounded-3xl sm:p-7">
          ...
          <div className="flex flex-col items-center"><label className="group relative cursor-pointer"><div className="h-28 w-28 overflow-hidden rounded-full bg-highlight ring-2 ring-border">{avatarPreview ? <img src={avatarPreview} alt="Profile preview" className="h-full w-full object-cover" /> : <UserAvatar user={profile} size="h-full w-full" className="text-4xl" />}</div><span className="absolute bottom-0 right-0 grid h-9 w-9 place-items-center rounded-full bg-primary-label text-primary-background shadow-lg"><Camera className="h-4 w-4" /></span><input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(event) => setAvatarFile(event.target.files?.[0] || null)} /></label><p className="mt-3 text-xs text-secondary-label">Change profile picture</p></div>
