...
import { ArrowLeft, Check, Edit3, Play, UserPlus } from 'lucide-react';
        ...
        <section className="flex flex-col items-center gap-6 text-center sm:flex-row sm:items-start sm:text-left">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex flex-col items-center gap-3 sm:flex-row">
              ...
              {profile.id !== user?.id && <button onClick={toggleFollow} className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold ${following ? 'bg-shading' : 'bg-primary-label text-primary-background'}`}>{following ? <Check className="h-4 w-4" /> : <UserPlus className="h-4 w-4" />}{following ? 'Following' : 'Follow'}</button>}
            </div>
