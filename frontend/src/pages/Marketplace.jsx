...
export default function Marketplace({ user }) {
  ...
  return <div className="min-h-screen bg-primary-background px-4 pb-24 text-primary-label sm:px-8">
    ...
    <main className="mx-auto max-w-5xl py-8"><div className="mb-6 flex justify-center"><button type="button" onClick={() => { setStatus(String()); setShowForm(true); }} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#34483B] px-5 text-sm font-semibold text-[#F4EBDD] shadow-lg transition hover:-translate-y-0.5"><Upload className="h-4 w-4" />List a beat</button></div><div className="grid gap-3 rounded-2xl border border-border bg-[#34483B] p-4 text-[#F4EBDD] sm:grid-cols-2 lg:grid-cols-6"><input value={filters.query} onChange={(e) => setFilters({ ...filters, query: e.target.value })} placeholder="Search marketplace" className="h-10 rounded-xl bg-[#F4EBDD]/10 px-3 text-sm outline-none placeholder:text-[#F4EBDD]/60 lg:col-span-2" /><select value={filters.category} onChange={(e) => setFilters({ ...filters, category: e.target.value })} className="h-10 rounded-xl bg-[#34483B] px-3 text-sm"><option value="all">All types</option>{CATEGORIES.map((item) => <option key={item} value={item}>{item}</option>)}</select><select value={filters.genre} onChange={(e) => setFilters({ ...filters, genre: e.target.value })} className="h-10 rounded-xl bg-[#34483B] px-3 text-sm"><option value="all">All genres</option>{GENRES.map((item) => <option key={item} value={item}>{item}</option>)}</select><select value={filters.bpm} onChange={(e) => setFilters({ ...filters, bpm: e.target.value })} className="h-10 rounded-xl bg-[#34483B] px-3 text-sm"><option value="all">Any tempo</option><option value="slow">Under 100 BPM</option><option value="mid">100-140 BPM</option><option value="fast">Over 140 BPM</option></select><select value={filters.instrument} onChange={(e) => setFilters({ ...filters, instrument: e.target.value })} className="h-10 rounded-xl bg-[#34483B] px-3 text-sm"><option value="all">All instruments</option>{INSTRUMENTS.map((item) => <option key={item} value={item}>{item}</option>)}</select></div>
      <section><h2 className="text-4xl font-bold tracking-tight">Find the next sound.</h2><p className="mt-2 max-w-xl text-secondary-label">Buy beats from independent creators with clear licensing and downloadable agreements.</p><div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{filteredBeats.length ? filteredBeats.map((beat) => <article key={beat.id} className="relative rounded-3xl border border-border bg-[#34483B] p-5 text-[#F4EBDD]">{beat.sellerId === user?.id && <div className="absolute right-3 top-3 z-10"><button type="button" onClick={() => setOpenActionId((current) => current === beat.id ? null : beat.id)} className="grid h-9 w-9 place-items-center rounded-xl bg-shading text-primary-label hover:bg-highlight" aria-label="Marketplace listing actions" aria-expanded={openActionId === beat.id}><MoreHorizontal className="h-5 w-5" /></button>{openActionId === beat.id && <div className="absolute right-0 top-11 min-w-36 rounded-xl border border-border bg-primary-background p-1 shadow-xl"><button type="button" onClick={() => setPendingDeleteBeat(beat)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-red-500 hover:bg-highlight"><Trash2 className="h-4 w-4" />Delete listing</button></div>}</div>}<div className="flex items-start justify-between gap-4 pr-8"><div className="min-w-0"><h3 className="truncate text-lg font-bold">{beat.title}</h3><p className="mt-1 text-sm text-[#F4EBDD]/75">@{beat.sellerUsername || 'seller'} · {beat.genre || 'Beat'} · {beat.bpm || '--'} BPM · {beat.key || '--'}</p></div></div><audio className="mt-4 w-full" controls preload="none" onTimeUpdate={(event) => { if (event.currentTarget.currentTime >= 60) { event.currentTarget.pause(); event.currentTarget.currentTime = 0; } }} src={beat.beatUrl?.startsWith('/') ? `${apiUrl}${beat.beatUrl.replace('/api/media/uploads/', '/uploads/')}` : beat.beatUrl} /></article>) : <div className="rounded-3xl border border-border bg-shading/40 p-6"><p className="text-sm text-secondary-label">No beats listed yet. Publish the first one.</p></div>}</div></section>
