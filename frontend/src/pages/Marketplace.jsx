...
import { ArrowLeft, Pause, Play, MoreHorizontal, Trash2, Upload, Store, X, Plus, Bookmark, ShoppingCart, BriefcaseBusiness } from 'lucide-react';
...
export default function Marketplace({ user }) {
  ...
  return <div className="min-h-screen bg-primary-background px-4 pb-24 text-primary-label sm:px-8">
    <header className="mx-auto flex max-w-5xl items-center justify-between py-5">
      ...
      <div className="flex items-center gap-2"><button type="button" onClick={() => setCartOpen(true)} className="relative grid h-10 w-10 place-items-center rounded-2xl bg-[#292929] text-[#F4EBDD] shadow-lg transition hover:-translate-y-0.5 hover:bg-[#3a3a3a]" aria-label="Open cart"><ShoppingCart className="h-5 w-5" />{cart.length > 0 && <span style={{ color: "#34483B", backgroundColor: "#F4EBDD" }} className="absolute -right-1 -top-1 z-10 grid h-5 min-w-5 place-items-center rounded-full px-1 text-xs font-bold leading-none">{String(cart.length)}</span>}</button><button type="button" onClick={() => setBackroomOpen(true)} className="grid h-10 w-10 place-items-center rounded-2xl bg-[#292929] text-[#F4EBDD] shadow-lg transition hover:-translate-y-0.5 hover:bg-[#3a3a3a]" aria-label="Open Backroom"><BriefcaseBusiness className="h-5 w-5" /></button></div>
    </header>
    <main className="mx-auto max-w-5xl py-8">
      <section><h2 className="text-4xl font-bold tracking-tight">Find the next sound.</h2><p className="mt-2 max-w-xl text-secondary-label">Buy beats from independent creators with clear licensing and downloadable agreements.</p><div className="mt-7 grid gap-3 rounded-2xl border border-border bg-[#292929] p-4 text-[#F4EBDD] sm:grid-cols-2 lg:grid-cols-6"><input value={filters.query} onChange={(e) => setFilters({ ...filters, query: e.target.value })} placeholder="Search marketplace" className="h-10 rounded-xl bg-[#F4EBDD]/10 px-3 text-sm outline-none placeholder:text-[#F4EBDD]/60 lg:col-span-2" /><select value={filters.category} onChange={(e) => setFilters({ ...filters, category: e.target.value })} className="h-10 rounded-xl bg-[#292929] px-3 text-sm"><option value="all">All types</option>{CATEGORIES.map((item) => <option key={item} value={item}>{item[0].toUpperCase() + item.slice(1)}</option>)}</select><select value={filters.genre} onChange={(e) => setFilters({ ...filters, genre: e.target.value })} className="h-10 rounded-xl bg-[#292929] px-3 text-sm"><option value="all">All genres</option>{GENRES.map((item) => <option key={item} value={item}>{item}</option>)}</select><select value={filters.bpm} onChange={(e) => setFilters({ ...filters, bpm: e.target.value })} className="h-10 rounded-xl bg-[#292929] px-3 text-sm"><option value="all">Any tempo</option><option value="slow">Under 100 BPM</option><option value="mid">100-140 BPM</option><option value="fast">Over 140 BPM</option></select><select value={filters.instrument} onChange={(e) => setFilters({ ...filters, instrument: e.target.value })} className="h-10 rounded-xl bg-[#292929] px-3 text-sm"><option value="all">All instruments</option>{INSTRUMENTS.map((item) => <option key={item} value={item}>{item}</option>)}</select></div><div className="my-6 flex justify-center"><button type="button" onClick={() => { setStatus(String()); setShowForm(true); }} className="grid h-10 w-10 place-items-center rounded-xl bg-[#292929] text-[#F4EBDD] shadow-lg transition hover:-translate-y-0.5" aria-label="Add marketplace listing"><Plus className="h-5 w-5" /></button></div><div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{filteredBeats.length ? filteredBeats.map((beat) => <article key={beat.id} className="relative rounded-3xl border border-border bg-[#292929] p-5 text-[#F4EBDD]">{beat.sellerId === user?.id && <div className="absolute right-3 top-3 z-10"><button type="button" onClick={() => setOpenActionId((current) => current === beat.id ? null : beat.id)} className="grid h-9 w-9 place-items-center rounded-xl bg-shading text-primary-label hover:bg-highlight" aria-label="Marketplace listing actions" aria-expanded={openActionId === beat.id}><MoreHorizontal className="h-5 w-5" /></button>{openActionId === beat.id && <div className="absolute right-0 top-11 min-w-36 rounded-xl border border-border bg-primary-background p-1 shadow-xl"><button type="button" onClick={() => setPendingDeleteBeat(beat)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-red-500 hover:bg-highlight"><Trash2 className="h-4 w-4" />Delete listing</button></div>}</div>}<div className="flex items-start justify-between gap-4 pr-8"><div className="min-w-0"><h3 className="truncate text-lg font-bold">{beat.title}</h3><p className="mt-1 text-sm text-[#F4EBDD]/75">@{beat.sellerUsername || 'seller'} · {beat.genre || 'Beat'} · {beat.bpm || '--'} BPM · {beat.key || '--'}</p><p className="mt-2 text-base font-bold text-[#F4EBDD]">₦{Number(beat.price || 0).toLocaleString('en-NG')}</p></div></div><button type="button" onClick={async () => { const response = await fetch(`${apiUrl}/api/marketplace/beats/${beat.id}/save`, { method: "POST", credentials: "include" }); if (response.ok) { const data = await response.json(); setSavedBeatIds((current) => data.saved ? [...new Set([...current, beat.id])] : current.filter((id) => id !== beat.id)); } }} className="mb-2 inline-flex items-center gap-2 text-sm font-semibold text-[#F4EBDD]" aria-label={savedBeatIds.includes(beat.id) ? "Unsave beat" : "Save beat"}><Bookmark className={`h-4 w-4 ${savedBeatIds.includes(beat.id) ? "text-[#F4EBDD]" : "text-[#34483B]"}`} fill={savedBeatIds.includes(beat.id) ? "currentColor" : "none"} />{savedBeatIds.includes(beat.id) ? "Saved" : "Save"}</button><button type="button" onClick={() => addToCart(beat)} disabled={cart.some((item) => item.id === beat.id)} className="mb-2 ml-3 inline-flex items-center gap-2 rounded-xl bg-transparent px-3 py-2 text-sm font-semibold text-[#F4EBDD] disabled:opacity-60"><ShoppingCart className="h-4 w-4" />{cart.some((item) => item.id === beat.id) ? 'In cart' : 'Add to cart'}</button><audio className="mt-4 w-full" controls preload="none" onTimeUpdate={(event) => { if (event.currentTarget.currentTime >= 60) { event.currentTarget.pause(); event.currentTarget.currentTime = 0; } }} src={beat.beatUrl?.startsWith('/') ? `${apiUrl}${beat.beatUrl.replace('/api/media/uploads/', '/uploads/')}` : beat.beatUrl} /></article>) : <div className="rounded-3xl border border-border bg-shading/40 p-6"><p className="text-sm text-secondary-label">No beats listed yet. Publish the first one.</p></div>}</div></section>

      {backroomOpen && <div className="fixed inset-0 z-50 bg-black/20" onClick={() => setBackroomOpen(false)}><aside className="absolute right-4 top-20 w-[min(24rem,calc(100vw-2rem))] rounded-2xl bg-[#34483B] p-5 text-[#F4EBDD] shadow-2xl" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between"><h2 className="text-xl font-bold">Backroom</h2><button type="button" onClick={() => setBackroomOpen(false)} aria-label="Close Backroom"><X className="h-5 w-5" /></button></div><p className="mt-1 text-sm text-[#F4EBDD]/70">Your marketplace operations.</p><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl bg-[#F4EBDD]/10 p-3"><p className="text-xs text-[#F4EBDD]/70">Listings</p><p className="mt-1 text-2xl font-bold">{beats.filter((item) => item.sellerId === user?.id).length}</p></div><div className="rounded-xl bg-[#F4EBDD]/10 p-3"><p className="text-xs text-[#F4EBDD]/70">Total sales</p><p className="mt-1 text-2xl font-bold">₦0</p></div></div><div className="mt-4 space-y-2 rounded-xl border border-[#F4EBDD]/20 p-3 text-sm"><p className="font-semibold">Sales tracking</p><p className="text-[#F4EBDD]/70">No completed sales yet.</p></div><div className="mt-3 space-y-2 rounded-xl border border-[#F4EBDD]/20 p-3 text-sm"><p className="font-semibold">Withdraw earnings</p><p className="text-[#F4EBDD]/70">Withdrawals unlock after verified sales and secure payout setup.</p><button type="button" disabled className="mt-2 w-full rounded-xl bg-[#F4EBDD]/20 px-3 py-2 font-semibold opacity-60">Set up payouts</button></div></aside></div>}      {cartOpen && <div className="fixed inset-0 z-50 bg-black/20" onClick={() => setCartOpen(false)}><aside className="absolute right-4 top-20 w-80 rounded-2xl bg-[#34483B] p-5 text-[#F4EBDD] shadow-2xl"><h2 className="text-lg font-bold">Your cart</h2>{cart.length === 0 ? <p className="py-8 text-sm">Your cart is empty.</p> : cart.map((item) => <div key={item.id} className="mt-3 border-b border-[#F4EBDD]/20 pb-3"><p className="font-semibold">{item.title}</p><p className="text-xs">@{item.sellerUsername || 'seller'} · {item.licenseType}</p><p className="font-bold">₦{Number(item.price || 0).toLocaleString('en-NG')}</p><button type="button" onClick={() => removeFromCart(item.id)} className="text-xs">Remove</button></div>)}</aside></div>}<ConfirmModal isOpen={Boolean(pendingDeleteBeat)} onClose={() => setPendingDeleteBeat(null)} onConfirm={() => deleteBeat(pendingDeleteBeat)} title="Delete marketplace listing?" message="This will remove the listing and its uploaded beat files. This action cannot be undone." confirmText="Delete listing" />
