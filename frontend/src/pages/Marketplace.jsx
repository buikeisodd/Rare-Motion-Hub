...
export default function Marketplace({ user }) {
  ...
  return <div className="min-h-screen bg-primary-background px-4 pb-24 text-primary-label sm:px-8">
    <header className="mx-auto flex max-w-5xl items-center justify-between py-5">
      ...
      <div className="flex items-center gap-2"><button type="button" onClick={() => setCartOpen(true)} className="relative grid h-10 w-10 place-items-center rounded-xl bg-[#34483B] text-[#F4EBDD]" aria-label="Open cart"><ShoppingCart className="h-5 w-5" />{cart.length > 0 && <span style={{ color: "#34483B", backgroundColor: "#F4EBDD" }} className="absolute -right-1 -top-1 z-10 grid h-5 min-w-5 place-items-center rounded-full px-1 text-xs font-bold leading-none">{String(cart.length)}</span>}</button><button type="button" onClick={() => setBackroomOpen(true)} className="grid h-10 w-10 place-items-center rounded-xl bg-[#34483B] text-[#F4EBDD]" aria-label="Open Backroom"><BriefcaseBusiness className="h-5 w-5" /></button></div>
    </header>
