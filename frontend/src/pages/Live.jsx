...
export default function Live({ user }) {
  ...
  const sendLiveData = async (event) => { if (!roomRef.current || !connected) return; await roomRef.current.localParticipant.publishData(new TextEncoder().encode(JSON.stringify({ ...event, sender: user?.username || user?.name || 'Listener' })), { reliable: true }); };
  const sendChat = async (event) => { event.preventDefault(); if (!chatText.trim()) return; const text = chatText.trim(); setChatText(''); const item = { type: 'chat', text, sender: user?.username || user?.name || 'Listener' }; setLiveMessages((current) => [...current.slice(-49), item]); await sendLiveData(item); };
