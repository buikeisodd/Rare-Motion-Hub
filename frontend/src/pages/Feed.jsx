...
function FeedCard({ item, user, onUpdate, onDelete, muted, onMutedChange, volume, onVolumeChange }) {
  ...
  const toggleLike = async () => { const next = !item.likedByMe; onUpdate(item.id, { likedByMe: next, likeCount: Math.max(0, (item.likeCount || 0) + (next ? 1 : -1)) }, 'like'); const res = await fetch(`${apiUrl}/api/feed/tracks/${item.id}/like`, { method: 'POST', ...mutationOptions() }); if (!res.ok) onUpdate(item.id, { likedByMe: !next, likeCount: item.likeCount || 0 }, 'like'); };
  const toggleSave = async () => { const next = !item.savedByMe; onUpdate(item.id, { savedByMe: next }, 'save'); const res = await fetch(`${apiUrl}/api/feed/tracks/${item.id}/save`, { method: 'POST', ...mutationOptions() }); if (!res.ok) onUpdate(item.id, { savedByMe: !next }, 'save'); };
