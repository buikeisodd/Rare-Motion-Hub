...
import { ArrowLeft, Bookmark, Heart, Library, MessageCircle, MoreHorizontal, Pause, Play, Plus, Radio, Search, Send, Settings, Store, Trash2, UserRound, Volume2, VolumeX, X } from 'lucide-react';
...
function StoryViewer({ story, stories, user, onClose, onNavigate, onDelete, onStoryLiked }) {
  ...
  const confirmStoryDelete = async () => { try { const deleted = await onDelete(story.id); if (!deleted) throw new Error('Could not delete story.'); setDeleteConfirmOpen(false); setDeleteFeedback('Story deleted successfully.'); window.setTimeout(onClose, 700); } catch (error) { setDeleteConfirmOpen(false); setDeleteFeedback(error.message || 'Could not delete story.'); } };
  ...
  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[#34483B]/75 p-3 backdrop-blur-2xl sm:p-6" onClick={onClose}>
    ...
    {deleteFeedback && <div role="status" className="absolute bottom-20 left-1/2 z-40 -translate-x-1/2 rounded-xl bg-[#F3EBDD]/95 px-4 py-2 text-sm font-semibold text-[#34483B] shadow-xl">{deleteFeedback}</div>}
    <ConfirmModal isOpen={deleteConfirmOpen} onClose={() => setDeleteConfirmOpen(false)} onConfirm={confirmStoryDelete} title="Delete story?" message="This story will be removed for everyone and cannot be restored." confirmText="Delete story" />
  </div>;
}
function StoryRail({ stories, onCreate, onDelete, onOpen, user }) { const grouped = Object.values(stories.reduce((groups, story) => { const key = story.owner?.id || story.userId || story.id; (groups[key] ||= []).push(story); return groups; }, {})); return <section className="mb-7" aria-label="Stories"><div className="flex gap-3 overflow-x-auto pb-1"><div className="w-20 shrink-0 text-center"><button onClick={onCreate} className="grid h-28 w-20 place-items-center rounded-2xl border border-border bg-shading/40 text-[#34483B]/70 transition hover:-translate-y-0.5 hover:bg-highlight"><Plus className="h-6 w-6" /></button><span className="mt-2 block truncate text-[10px] font-semibold text-[#34483B]">Create</span></div>{grouped.map((group) => { const story = group[0]; const ownerId = story.owner?.id || story.userId || story.id; const latest = group.reduce((value, item) => Math.max(value, new Date(item.createdAt || 0).getTime()), 0); const viewerId = user?.id || user?._id || 'anonymous'; const seenKey = `rmh:story-seen:${viewerId}:${ownerId}`; const seenAt = Number(localStorage.getItem(seenKey) || 0); const isNew = latest > seenAt; return <div key={ownerId} className="w-20 shrink-0 text-center"><button onClick={() => { localStorage.setItem(seenKey, String(latest)); onOpen({ ...story, storyGroup: group }); }} className={`group relative block h-28 w-20 overflow-hidden rounded-2xl border bg-shading text-left transition-shadow ${isNew ? 'border-[#718A78] shadow-[0_0_0_3px_rgba(113,138,120,.28),0_0_22px_rgba(113,138,120,.72)]' : 'border-border'}`}><div className="h-full w-full">{story.contentType === 'text' ? <div className="flex h-full items-center justify-center bg-[#718A78] p-2 text-center text-xs text-[#F3EBDD]">{story.text}</div> : story.project?.coverArt ? <img src={story.project.coverArt} alt="" className="h-full w-full object-cover" /> : <div className={`h-full w-full bg-gradient-to-br ${gradientFor(story.id) || defaultGradient}`} />}</div></button><span className="mt-2 block truncate text-[10px] font-semibold text-[#34483B]">{ownerId === user?.id ? 'You' : story.owner?.name || 'Artist'}</span></div>; })}</div></section>; }
...
function CreateModal({ open, onClose, projects, onCreated, onFeedCreate }) {
  ...
  return <div className="fixed inset-0 z-[80] grid place-items-center bg-[#34483B]/45 p-4 backdrop-blur-xl"><div className="create-flow-surface w-full max-w-lg rounded-3xl border border-white/30 p-5 shadow-2xl"><div className="mb-5 flex items-center justify-between"><h2 className="font-display text-xl font-bold">Create</h2><button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-xl bg-[#E4E0D4]" aria-label="Close"><X className="h-4 w-4" /></button></div>{step === 'destination' && <div className="grid gap-3"><p className="text-sm opacity-70">Where would you like to publish?</p><button onClick={() => setStep('kind')} className="rounded-2xl bg-[#D9DED0] p-4 text-left font-semibold hover:bg-[#C8D2C4]">Story<span className="mt-1 block text-xs font-normal opacity-70">Share a note or a 40-second preview.</span></button><button onClick={() => { onClose(); onFeedCreate?.(); }} className="rounded-2xl border border-[#718A78]/30 p-4 text-left font-semibold">Feed<span className="mt-1 block text-xs font-normal opacity-70">Create a feed post from a project preview.</span></button></div>}{step === 'kind' && <div className="grid gap-3"><p className="text-sm opacity-70">What would you like to share?</p><button onClick={() => { setType('text'); setStep('text'); }} className="rounded-2xl bg-[#D9DED0] p-4 text-left font-semibold hover:bg-[#C8D2C4]">Note / text</button><button onClick={() => { setType('track'); setStep('tracks'); }} className="rounded-2xl bg-[#D9DED0] p-4 text-left font-semibold hover:bg-[#C8D2C4]">Track preview</button></div>}{step === 'text' && <div className="space-y-4"><textarea autoFocus value={text} onChange={(event) => setText(event.target.value)} maxLength={1000} placeholder="Write your story..." className="min-h-40 w-full resize-none rounded-2xl border border-[#718A78]/30 bg-white/40 p-4 outline-none" />{error && <p className="text-sm text-red-600">{error}</p>}<button onClick={publish} disabled={saving} className="w-full rounded-2xl bg-[#718A78] px-4 py-3 font-semibold text-[#F3EBDD] disabled:opacity-50">{saving ? 'Publishing...' : 'Post to story'}</button></div>}{step === 'tracks' && <div className="max-h-72 space-y-2 overflow-y-auto">{tracks.length ? tracks.map((item) => <button key={item.id} onClick={() => { setTrack(item); setStep('preview'); }} className="flex w-full items-center justify-between rounded-2xl bg-[#D9DED0] p-3 text-left hover:bg-[#C8D2C4]"><span className="truncate font-semibold">{item.title || item.filename || 'Untitled track'}</span><span className="ml-3 shrink-0 text-xs opacity-70">{item.project.title || item.project.name}</span></button>) : <p className="py-8 text-center text-sm opacity-70">No tracks available yet.</p>}</div>}{step === 'preview' && track && <div className="space-y-4"><h3 className="font-semibold">{track.title || track.filename}</h3><p className="text-sm opacity-70">Your story will play automatically for up to 40 seconds.</p>{error && <p className="text-sm text-red-600">{error}</p>}<button onClick={publish} disabled={saving} className="w-full rounded-2xl bg-[#718A78] px-4 py-3 font-semibold text-[#F3EBDD] disabled:opacity-50">{saving ? 'Publishing...' : 'Post preview to story'}</button></div>}</div></div>;
}

function FeedPublishModal({ open, onClose, projects, onPublished }) {
  const [track, setTrack] = useState(null); const [caption, setCaption] = useState(''); const [start, setStart] = useState(0); const [end, setEnd] = useState(''); const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  useEffect(() => { if (!open) { setTrack(null); setCaption(''); setStart(0); setEnd(''); setError(''); } }, [open]);
  if (!open) return null;
  const tracks = projects.flatMap((project) => (project.tracks || []).map((item) => ({ ...item, project })));
  const choose = (item) => { setTrack(item); setStart(0); setEnd(Math.min(Number(item.duration) || 40, 40)); };
  const publish = async () => { if (!track) return setError('Choose a track first.'); setSaving(true); setError(''); try { const res = await fetch(`${apiUrl}/api/tracks/${track.id}/publish`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ published: true, caption: caption.trim(), previewStart: Number(start), previewEnd: Number(end) || null }) }); const data = await res.json().catch(() => ({})); if (!res.ok) throw new Error(data.error || 'Could not publish preview.'); onPublished?.(data.track); onClose(); } catch (err) { setError(err.message); } finally { setSaving(false); } };
  return <div className="create-flow-backdrop fixed inset-0 z-[90] grid place-items-center bg-[#34483B]/45 p-4 backdrop-blur-xl"><div className="create-flow-surface w-full max-w-lg rounded-3xl border border-white/30 p-5 shadow-2xl"><div className="mb-5 flex items-center justify-between"><div><h2 className="font-display text-xl font-bold">Publish to feed</h2><p className="mt-1 text-sm opacity-70">Choose a track, caption, and listening moment.</p></div><button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-xl bg-[#E4E0D4]" aria-label="Close"><X className="h-4 w-4" /></button></div>{!track ? <div className="max-h-72 space-y-2 overflow-y-auto">{tracks.length ? tracks.map((item) => <button key={item.id} onClick={() => choose(item)} className="flex w-full items-center justify-between rounded-2xl bg-[#D9DED0] p-3 text-left hover:bg-[#C8D2C4]"><span className="truncate font-semibold">{item.title || item.filename || 'Untitled track'}</span><span className="ml-3 shrink-0 text-xs opacity-70">{item.project.title || item.project.name}</span></button>) : <p className="py-8 text-center text-sm opacity-70">No tracks available yet.</p>}</div> : <div className="space-y-4"><div className="flex items-center justify-between gap-3"><h3 className="min-w-0 truncate font-semibold">{track.title || track.filename || 'Untitled track'}</h3><button type="button" onClick={() => setTrack(null)} className="shrink-0 text-xs font-semibold underline">Change track</button></div><textarea value={caption} onChange={(event) => setCaption(event.target.value)} maxLength={500} placeholder="Add a caption" className="min-h-24 w-full resize-none rounded-2xl border border-[#718A78]/30 bg-white/40 p-3 outline-none" /><div className="grid grid-cols-2 gap-3"><label className="text-xs font-semibold">Start (seconds)<input type="number" min="0" value={start} onChange={(event) => setStart(event.target.value)} className="mt-1 w-full rounded-xl border border-[#718A78]/30 bg-white/40 p-2.5" /></label><label className="text-xs font-semibold">End (seconds)<input type="number" min="1" value={end} onChange={(event) => setEnd(event.target.value)} className="mt-1 w-full rounded-xl border border-[#718A78]/30 bg-white/40 p-2.5" /></label></div>{error && <p className="text-sm text-red-600">{error}</p>}<button onClick={publish} disabled={saving} className="w-full rounded-2xl bg-[#718A78] px-4 py-3 font-semibold text-[#F3EBDD] disabled:opacity-50">{saving ? 'Publishing...' : 'Publish preview'}</button></div>}</div></div>;
}
...
function FeedCard({ item, user, onUpdate, onDelete, muted, onMutedChange, volume, onVolumeChange }) {
  ...
  return <motion.article ref={cardRef} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="relative mx-auto w-full max-w-xl overflow-hidden rounded-2xl border border-border bg-shading/35">
    <header className="feed-post-header absolute inset-x-0 top-0 z-20 flex items-center gap-3 border-b border-[#F3EBDD]/15 bg-[#34483B]/78 px-4 py-3 text-[#F3EBDD] shadow-[0_8px_22px_rgba(52,72,59,.38)] backdrop-blur-xl">
      ...
      <div className="relative"><button onClick={() => setMenuOpen((value) => !value)} className="grid h-8 w-8 place-items-center rounded-xl bg-transparent text-[#F3EBDD] hover:bg-[#F3EBDD]/12 hover:text-[#F3EBDD]" aria-label="Post options"><MoreHorizontal className="h-5 w-5 !text-[#F3EBDD]" /></button>{menuOpen && <div className="absolute right-0 top-9 z-10 w-44 rounded-xl border border-[#F3EBDD]/15 bg-[#34483B]/90 p-1 text-[#F3EBDD] shadow-2xl backdrop-blur-xl">{item.owner?.id === user?.id ? <button onClick={() => { setMenuOpen(false); onDelete(item.id); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-red-400 hover:bg-red-500/10"><Trash2 className="h-4 w-4" />Delete preview</button> : <button onClick={() => setMenuOpen(false)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-[#F3EBDD] hover:bg-[#F3EBDD]/12">Not interested</button>}</div>}</div>
    </header>
    <div onClick={() => item.project?.id && navigate(`/project/${item.project.id}`)} className="group relative aspect-square w-full cursor-pointer overflow-hidden bg-black sm:aspect-[4/3]">{item.project?.coverArt ? <img src={item.project.coverArt} alt="" className="block h-full w-full object-cover" /> : <div className={`h-full w-full bg-gradient-to-br ${gradientFor(item.id) || defaultGradient}`} />}<button onClick={(event) => { event.stopPropagation(); playing ? stop() : play(); }} className={`absolute inset-0 m-auto grid h-12 w-12 place-items-center rounded-full bg-accent text-primary-background shadow-xl transition-opacity hover:scale-105 sm:opacity-0 sm:group-hover:opacity-100 sm:focus-visible:opacity-100 ${playing ? 'opacity-100' : ''}`} aria-label={playing ? 'Pause preview' : 'Play preview'}>{playing ? <Pause className="h-4 w-4 fill-current" /> : <Play className="ml-1 h-4 w-4 fill-current" />}</button><div className="group/volume absolute bottom-3 right-3 flex items-center gap-1 rounded-full bg-[#34483B]/85 px-1 py-1 text-[#F3EBDD] shadow-lg backdrop-blur-md"><input type="range" min="0" max="1" step="0.01" value={muted ? 0 : volume} onChange={(event) => { event.stopPropagation(); onVolumeChange(Number(event.target.value)); }} onClick={(event) => event.stopPropagation()} className="h-1 w-0 max-w-[18vw] accent-[#F3EBDD] opacity-0 transition-all group-hover/volume:w-16 group-hover/volume:opacity-100 focus-visible:w-16 focus-visible:opacity-100" aria-label="Preview volume" /><button onClick={(event) => { event.stopPropagation(); toggleMute(); }} className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-transparent text-[#F3EBDD] hover:bg-[#F3EBDD]/12" aria-label={muted ? 'Unmute preview' : 'Mute preview'}>{muted ? <VolumeX className="h-3.5 w-3.5 !text-[#F3EBDD]" /> : <Volume2 className="h-3.5 w-3.5 !text-[#F3EBDD]" />}</button></div><span className="absolute bottom-3 left-3 rounded-full bg-[#34483B]/85 px-2.5 py-1 text-[11px] font-semibold text-[#F3EBDD] shadow-lg backdrop-blur-md">{playing ? 'Previewing' : 'Preview'}</span></div>
...
export default function Feed({ user, savedOnly = false }) {
  ...
  const [stories, setStories] = useState([]); const [createOpen, setCreateOpen] = useState(false); const [feedPublishOpen, setFeedPublishOpen] = useState(false); const [storyViewer, setStoryViewer] = useState(null); const [workspaceProjects, setWorkspaceProjects] = useState([]);
  ...
  const navItems = [{ label: 'pRoFiLe', icon: UserRound, to: '/profile/' + (user?.id || '') }, { label: 'sEaRcH', icon: Search, to: '/search' }, { label: 'lIvE', icon: Radio, to: '/live' }, { label: 'mArKeTpLaCe', icon: Store, to: '/marketplace' }, { label: 'liBraRy', icon: Library, to: '/library' }];
  return <div className="feed-shell min-h-screen bg-primary-background pb-24 text-primary-label md:pb-0">
    ...
    <CreateModal open={createOpen} onClose={() => setCreateOpen(false)} onFeedCreate={() => setFeedPublishOpen(true)} projects={workspaceProjects} onCreated={async () => { const response = await fetch(`${apiUrl}/api/stories`, { credentials: 'include' }); const data = await response.json().catch(() => ({})); if (response.ok) setStories(data.stories || []); }} />
    <FeedPublishModal open={feedPublishOpen} onClose={() => setFeedPublishOpen(false)} projects={workspaceProjects} onPublished={() => loadFeedPage()} />
