...
function StoryViewer({ story, stories, onClose, onNavigate }) { const audioRef = useRef(null); const [progress, setProgress] = useState(0); const sequence = story?.storyGroup || [story]; const index = Math.max(0, sequence.findIndex((item) => item.id === story?.id)); const ownerIndex = Math.max(0, stories.findIndex((item) => item.id === story?.storyGroup?.[0]?.id || item.id === story?.id)); const move = (offset) => { const next = index + offset; if (next >= 0 && next < sequence.length) onNavigate({ ...sequence[next], storyGroup: sequence }); else { const nextOwner = ownerIndex + offset; if (nextOwner >= 0 && nextOwner < stories.length) onNavigate(stories[nextOwner]); else onClose(); } }; const start = Number(story?.previewStart || 0); const maxEnd = Math.min(Number(story?.previewEnd || 40), start + 40); const duration = Math.max(1, maxEnd - start); useEffect(() => { setProgress(0); const audio = audioRef.current; if (story?.contentType === 'track' && audio) { const ready = () => { audio.currentTime = start; audio.play().catch(() => {}); }; audio.addEventListener('loadedmetadata', ready); if (audio.readyState >= 1) ready(); return () => { audio.removeEventListener('loadedmetadata', ready); audio.pause(); }; } const timer = window.setInterval(() => setProgress((value) => { const next = value + 100 / (duration * 10); if (next >= 100) { window.clearInterval(timer); move(1); return 100; } return next; }), 100); return () => window.clearInterval(timer); }, [story?.id]); if (!story) return null; return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[#34483B]/75 p-3 backdrop-blur-2xl sm:p-6" onClick={onClose}><button onClick={(event) => { event.stopPropagation(); move(-1); }} disabled={index <= 0 && ownerIndex <= 0} className="absolute left-3 grid h-11 w-11 place-items-center rounded-2xl bg-[#F3EBDD]/90 text-[#34483B] shadow-lg disabled:invisible sm:left-8" aria-label="Previous story"><ArrowLeft className="h-5 w-5" /></button><div onClick={(event) => event.stopPropagation()} className="relative flex h-[min(88vh,720px)] w-[min(92vw,420px)] flex-col overflow-hidden rounded-[2rem] border border-[#F3EBDD]/30 bg-[#718A78] shadow-2xl"><div className="absolute inset-x-4 top-4 z-10 flex gap-1">{sequence.map((item, itemIndex) => <div key={item.id} className="h-1 flex-1 rounded-full bg-[#F3EBDD]/40"><div className="h-full rounded-full bg-[#F3EBDD]" style={{ width: itemIndex < index ? '100%' : itemIndex === index ? `${progress}%` : '0%' }} /></div>)}</div><div className="relative z-10 mt-7 flex items-center justify-between px-5 text-[#F3EBDD]"><span className="truncate text-sm font-semibold">{story.owner?.name || 'Story'}</span><button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-xl bg-[#34483B]/40" aria-label="Close"><X className="h-5 w-5" /></button></div><div className="flex min-h-0 flex-1 items-center justify-center p-5 text-center">{story.contentType === 'text' ? <p className="max-h-full overflow-auto break-words text-2xl font-semibold text-[#F3EBDD]">{story.text}</p> : story.project?.coverArt ? <img src={story.project.coverArt} alt="" className="max-h-full w-full rounded-2xl object-cover" /> : <div className={`h-full w-full rounded-2xl bg-gradient-to-br ${gradientFor(story.id) || defaultGradient}`} />}</div>{story.contentType === 'track' && <audio ref={audioRef} preload="auto" src={story.url} onTimeUpdate={(e) => { const value = Math.max(0, e.currentTarget.currentTime - start); setProgress(Math.min(100, value / duration * 100)); if (e.currentTarget.currentTime >= maxEnd) { e.currentTarget.pause(); move(1); } }} onEnded={() => move(1)} />}{<div className="absolute inset-x-5 bottom-5 rounded-full border border-[#F3EBDD]/70 bg-[#34483B]/30 px-4 py-2 text-sm text-[#F3EBDD]/80">Reply to story...</div>}</div><button onClick={(event) => { event.stopPropagation(); const nextOwner = ownerIndex + 1; if (nextOwner < stories.length) onNavigate(stories[nextOwner]); else onClose(); }} disabled={ownerIndex >= stories.length - 1} className="absolute right-3 grid h-11 w-11 place-items-center rounded-2xl bg-[#F3EBDD]/90 text-[#34483B] shadow-lg disabled:invisible sm:right-8" aria-label="Next story"><ArrowLeft className="h-5 w-5 rotate-180" /></button></div>; }
function StoryRail({ stories, onCreate, onDelete, onOpen, user }) { return <section className="mb-7" aria-label="Stories"><div className="flex gap-3 overflow-x-auto pb-1"><div className="w-20 shrink-0 text-center"><button onClick={onCreate} className="grid h-28 w-20 place-items-center rounded-2xl border border-border bg-shading/40 text-[#34483B]/70 transition hover:-translate-y-0.5 hover:bg-highlight"><Plus className="h-6 w-6" /></button><span className="mt-2 block truncate text-[10px] font-semibold text-[#34483B]">Create</span></div>{stories.map((story) => <div key={story.id} className="w-20 shrink-0 text-center"><button onClick={() => onOpen(story)} className="group relative block h-28 w-20 overflow-hidden rounded-2xl border border-border bg-shading text-left"><div className="h-full w-full">{story.contentType === 'text' ? <div className="flex h-full items-center justify-center bg-[#718A78] p-2 text-center text-xs text-[#F3EBDD]">{story.text}</div> : story.project?.coverArt ? <img src={story.project.coverArt} alt="" className="h-full w-full object-cover" /> : <div className={`h-full w-full bg-gradient-to-br ${gradientFor(story.id) || defaultGradient}`} />}</div>{story.owner?.id === user?.id && <span onClick={(e) => { e.stopPropagation(); onDelete(story.id); }} className="absolute right-1 top-1 grid h-5 w-5 place-items-center rounded-full bg-[#34483B]/70 text-[#F3EBDD] opacity-0 transition group-hover:opacity-100"><X className="h-3 w-3" /></span>}</button><span className="mt-2 block truncate text-[10px] font-semibold text-[#34483B]">{story.owner?.id === user?.id ? 'You' : story.owner?.name || 'Artist'}</span></div>)}</div></section>; }
