...
function StoryViewer({ story, stories, onClose, onNavigate }) { const audioRef = useRef(null); const index = Math.max(0, stories.findIndex((item) => item.id === story?.id)); const move = (offset) => { const next = index + offset; if (next >= 0 && next < stories.length) onNavigate(stories[next]); }; useEffect(() => { if (story?.contentType === 'track' && audioRef.current) { audioRef.current.currentTime = story.previewStart || 0; audioRef.current.play().catch(() => {}); } return () => audioRef.current?.pause(); }, [story]); useEffect(() => { const timer = window.setTimeout(() => { if (story?.contentType === 'text' || !story?.url) move(1); }, story?.contentType === 'text' ? 5000 : 90000); return () => window.clearTimeout(timer); }, [story?.id]); if (!story) return null; const maxEnd = Math.min(Number(story.previewEnd || 90), Number(story.previewStart || 0) + 90); return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[#34483B]/75 p-3 backdrop-blur-2xl sm:p-6"><button onClick={() => move(-1)} disabled={index <= 0} className="absolute left-3 grid h-11 w-11 place-items-center rounded-2xl bg-[#F3EBDD]/90 text-[#34483B] shadow-lg disabled:invisible sm:left-8" aria-label="Previous story"><ArrowLeft className="h-5 w-5" /></button><div className="relative flex h-[min(88vh,720px)] w-[min(92vw,420px)] flex-col overflow-hidden rounded-[2rem] border border-[#F3EBDD]/30 bg-[#718A78] shadow-2xl"><div className="absolute inset-x-4 top-4 z-10 h-1 rounded-full bg-[#F3EBDD]/40"><div className="h-full w-1/2 rounded-full bg-[#F3EBDD]" /></div><div className="relative z-10 mt-7 flex items-center justify-between px-5 text-[#F3EBDD]"><span className="truncate text-sm font-semibold">{story.owner?.name || 'Story'}</span><button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-xl bg-[#34483B]/40" aria-label="Close"><X className="h-5 w-5" /></button></div><div className={`flex min-h-0 flex-1 items-center justify-center p-5 text-center ${story.displayStyle === 'zoom' ? 'story-display-zoom' : ''}`}>{story.contentType === 'text' ? <p className="max-h-full overflow-auto break-words text-2xl font-semibold text-[#F3EBDD]">{story.text}</p> : story.project?.coverArt ? <img src={story.project.coverArt} alt="" className="max-h-full w-full rounded-2xl object-cover" /> : <div className={`h-full w-full rounded-2xl bg-gradient-to-br ${gradientFor(story.id) || defaultGradient}`} />}</div>{story.contentType === 'track' && <audio ref={audioRef} src={story.url} onTimeUpdate={(e) => { if (e.currentTarget.currentTime >= maxEnd) { e.currentTarget.pause(); move(1); } }} />}{story.caption && <p className="absolute inset-x-5 bottom-16 rounded-xl bg-[#34483B]/55 p-3 text-sm text-[#F3EBDD]">{story.caption}</p>}<div className="absolute inset-x-5 bottom-5 rounded-full border border-[#F3EBDD]/70 bg-[#34483B]/30 px-4 py-2 text-sm text-[#F3EBDD]/80">Reply to story...</div></div><button onClick={() => move(1)} disabled={index >= stories.length - 1} className="absolute right-3 grid h-11 w-11 place-items-center rounded-2xl bg-[#F3EBDD]/90 text-[#34483B] shadow-lg disabled:invisible sm:right-8" aria-label="Next story"><ArrowLeft className="h-5 w-5 rotate-180" /></button></div>; }
function StoryRail({ stories, onDelete, onOpen, user }) { return <section className="mb-7" aria-label="Stories"><div className="flex gap-3 overflow-x-auto pb-1">{stories.map((story) => <button key={story.id} onClick={() => onOpen(story)} className="group relative h-28 w-20 shrink-0 overflow-hidden rounded-2xl border border-border bg-shading text-left"><div className="h-full w-full">{story.contentType === 'text' ? <div className="flex h-full items-center justify-center bg-[#718A78] p-2 text-center text-xs text-[#F3EBDD]">{story.text}</div> : story.project?.coverArt ? <img src={story.project.coverArt} alt="" className="h-full w-full object-cover" /> : <div className={`h-full w-full bg-gradient-to-br ${gradientFor(story.id) || defaultGradient}`} />}</div><div className="absolute inset-x-1 bottom-1 truncate rounded-lg bg-[#34483B]/70 px-1.5 py-1 text-[10px] text-[#F3EBDD]">{story.owner?.id === user?.id ? 'You' : story.owner?.name || 'Artist'}</div>{story.owner?.id === user?.id && <span onClick={(e) => { e.stopPropagation(); onDelete(story.id); }} className="absolute right-1 top-1 grid h-5 w-5 place-items-center rounded-full bg-[#34483B]/70 text-[#F3EBDD] opacity-0 transition group-hover:opacity-100"><X className="h-3 w-3" /></span>}</button>)}</div></section>; }
  ...
  return <div className="min-h-screen bg-primary-background pb-24 text-primary-label md:pb-0">
    <aside className="group fixed bottom-4 left-4 top-4 z-40 hidden w-20 flex-col overflow-hidden rounded-3xl border border-white/10 bg-[#0a0a0a]/80 px-3 py-6 shadow-2xl backdrop-blur-2xl transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:w-64 hover:border-white/20 lg:flex">
      ...
      <nav className="space-y-2">
        <button onClick={() => setCreateOpen(true)} className="mb-2 flex h-12 w-full items-center whitespace-nowrap rounded-2xl px-3 text-sm font-semibold text-[#F3EBDD] transition-all hover:bg-accent/80 hover:backdrop-blur-sm hover:text-[#F3EBDD]" title="Create"><Plus className="h-6 w-6 shrink-0" /><span className="font-display ml-4 opacity-0 transition-opacity duration-300 group-hover:opacity-100">cReAtE</span></button>
    ...
    <main className="px-4 pb-20 pt-4 sm:px-8 lg:ml-28 xl:mr-80">
      <div className="mx-auto max-w-2xl py-3">
        ...
        <StoryRail stories={stories} user={user} onOpen={setStoryViewer} onDelete={async (id) => { const res = await fetch(`${apiUrl}/api/stories/${id}`, { method: 'DELETE' }); if (res.ok) setStories((current) => current.filter((story) => story.id !== id)); }} />
