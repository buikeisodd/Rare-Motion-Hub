...
function StoryViewer({ story, onClose }) { const audioRef = useRef(null); useEffect(() => { if (story?.contentType === 'track' && audioRef.current) { audioRef.current.currentTime = story.previewStart || 0; audioRef.current.play().catch(() => {}); } return () => audioRef.current?.pause(); }, [story]); if (!story) return null; return <div className="fixed inset-0 z-[90] grid place-items-center bg-black/75 p-4 backdrop-blur-xl"><div className="relative aspect-[9/16] h-[min(82vh,680px)] max-w-full overflow-hidden rounded-3xl bg-[#34483B] shadow-2xl"><div className="absolute inset-x-4 top-4 z-10 h-1 rounded-full bg-[#F3EBDD]/40"><div className="h-full w-1/2 rounded-full bg-[#F3EBDD]" /></div><button onClick={onClose} className="absolute right-4 top-8 z-10 grid h-9 w-9 place-items-center rounded-xl bg-black/30 text-[#F3EBDD]" aria-label="Close"><X className="h-5 w-5" /></button><div className={`flex h-full items-center justify-center p-8 text-center ${story.displayStyle === 'zoom' ? 'story-display-zoom' : ''}`}>{story.contentType === 'text' ? <p className="max-h-full overflow-auto break-words text-2xl font-semibold text-[#F3EBDD]">{story.text}</p> : story.project?.coverArt ? <img src={story.project.coverArt} alt="" className="h-full w-full object-cover" /> : <div className={`h-full w-full bg-gradient-to-br ${gradientFor(story.id) || defaultGradient}`} />}</div>{story.contentType === 'track' && <audio ref={audioRef} src={story.url} onTimeUpdate={(e) => { if (story.previewEnd && e.currentTarget.currentTime >= story.previewEnd) e.currentTarget.pause(); }} />}{story.caption && <p className="absolute inset-x-5 bottom-16 rounded-xl bg-black/30 p-3 text-sm text-[#F3EBDD]">{story.caption}</p>}<div className="absolute inset-x-5 bottom-5 rounded-full border border-[#F3EBDD]/60 px-4 py-2 text-sm text-[#F3EBDD]/80">Reply to story...</div></div></div>; }
function StoryRail({ stories, onCreate, onDelete, onOpen, user }) { return <section className="mb-7" aria-label="Stories"><div className="mb-3 flex items-center justify-between"><h2 className="font-display text-sm font-bold tracking-wider text-[#34483B]">sToRiEs</h2><button onClick={onCreate} className="inline-flex items-center gap-1.5 rounded-2xl px-3 py-2 text-xs font-semibold text-[#F3EBDD] transition-all hover:bg-highlight hover:text-[#34483B]"><Plus className="h-3.5 w-3.5" />Create</button></div><div className="flex gap-3 overflow-x-auto pb-1"><button onClick={onCreate} className="grid h-28 w-20 shrink-0 place-items-center rounded-2xl border border-border bg-shading/40 text-[#34483B]/70 transition hover:-translate-y-0.5 hover:bg-highlight"><Plus className="h-6 w-6" /><span className="text-[10px]">New story</span></button>{stories.map((story) => <button key={story.id} onClick={() => onOpen(story)} className="group relative h-28 w-20 shrink-0 overflow-hidden rounded-2xl border border-border bg-shading text-left"><div className="h-full w-full">{story.contentType === 'text' ? <div className="flex h-full items-center justify-center bg-[#718A78] p-2 text-center text-xs text-[#F3EBDD]">{story.text}</div> : story.project?.coverArt ? <img src={story.project.coverArt} alt="" className="h-full w-full object-cover" /> : <div className={`h-full w-full bg-gradient-to-br ${gradientFor(story.id) || defaultGradient}`} />}</div><div className="absolute inset-x-1 bottom-1 truncate rounded-lg bg-black/55 px-1.5 py-1 text-[10px] text-[#F3EBDD]">{story.owner?.id === user?.id ? 'You' : story.owner?.name || 'Artist'}</div>{story.owner?.id === user?.id && <span onClick={(e) => { e.stopPropagation(); onDelete(story.id); }} className="absolute right-1 top-1 grid h-5 w-5 place-items-center rounded-full bg-black/55 text-[#F3EBDD] opacity-0 transition group-hover:opacity-100"><X className="h-3 w-3" /></span>}</button>)}</div></section>; }
  ...
  return <div className="min-h-screen bg-primary-background pb-24 text-primary-label md:pb-0">
    ...
    <main className="px-4 pb-20 pt-4 sm:px-8 lg:ml-28 xl:mr-80">
      <div className="mx-auto max-w-2xl py-3">
        ...
        <StoryRail stories={stories} user={user} onOpen={setStoryViewer} onCreate={() => setCreateOpen(true)} onDelete={async (id) => { const res = await fetch(`${apiUrl}/api/stories/${id}`, { method: 'DELETE' }); if (res.ok) setStories((current) => current.filter((story) => story.id !== id)); }} />
    ...
    <StoryViewer story={storyViewer} onClose={() => setStoryViewer(null)} />
  </div>;
}
