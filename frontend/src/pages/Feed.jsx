...
function FeedCard({ item, user, onUpdate, onDelete, muted, onMutedChange, volume, onVolumeChange }) {
  ...
  return <motion.article ref={cardRef} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="relative mx-auto w-full max-w-xl overflow-hidden rounded-2xl border border-border bg-shading/35">
    <header className="feed-post-header absolute inset-x-0 top-0 z-20 flex items-center gap-3 border-b border-[#34483B]/12 bg-[#F3EBDD]/88 px-4 py-3 text-[#34483B] shadow-[0_8px_22px_rgba(52,72,59,.18)] backdrop-blur-xl">
      ...
      <div className="relative"><button onClick={() => setMenuOpen((value) => !value)} className="grid h-8 w-8 place-items-center rounded-xl bg-transparent text-[#34483B] hover:bg-[#D8E2D6] hover:text-[#34483B]" aria-label="Post options"><MoreHorizontal className="h-5 w-5 !text-[#34483B]" /></button>{menuOpen && <div className="absolute right-0 top-9 z-10 w-44 rounded-xl border border-[#34483B]/12 bg-[#F3EBDD]/95 p-1 text-[#34483B] shadow-2xl backdrop-blur-xl">{item.owner?.id === user?.id ? <button onClick={() => { setMenuOpen(false); onDelete(item.id); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-red-500 hover:bg-red-500/10"><Trash2 className="h-4 w-4" />Delete preview</button> : <button onClick={() => setMenuOpen(false)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-[#34483B] hover:bg-[#D8E2D6]">Not interested</button>}</div>}</div>
    </header>
