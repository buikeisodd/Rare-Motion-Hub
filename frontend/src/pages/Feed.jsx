...
function FeedCard({ item, user, onUpdate, onDelete, muted, onMutedChange, volume, onVolumeChange }) {
  ...
  return <motion.article ref={cardRef} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="relative mx-auto w-full max-w-xl overflow-hidden rounded-2xl border border-border bg-shading/35">
    ...
    <div onClick={() => item.project?.id && navigate(`/project/${item.project.id}`)} className="group relative aspect-square w-full cursor-pointer overflow-hidden bg-black sm:aspect-[4/3]">{item.project?.coverArt ? <img src={item.project.coverArt} alt="" className="block h-full w-full object-cover" /> : <div className={`h-full w-full bg-gradient-to-br ${gradientFor(item.id) || defaultGradient}`} />}<button onClick={(event) => { event.stopPropagation(); playing ? stop() : play(); }} className={`absolute inset-0 m-auto grid h-12 w-12 place-items-center rounded-full bg-accent text-primary-background shadow-xl transition-opacity hover:scale-105 sm:opacity-0 sm:group-hover:opacity-100 sm:focus-visible:opacity-100 ${playing ? 'opacity-100' : ''}`} aria-label={playing ? 'Pause preview' : 'Play preview'}>{playing ? <Pause className="h-4 w-4 fill-current" /> : <Play className="ml-1 h-4 w-4 fill-current" />}</button><div className="group/volume absolute bottom-3 right-3 flex items-center gap-2 rounded-full bg-[#34483B]/85 px-1.5 py-1.5 text-[#F3EBDD] shadow-lg backdrop-blur-md"><input type="range" min="0" max="1" step="0.01" value={muted ? 0 : volume} onChange={(event) => { event.stopPropagation(); onVolumeChange(Number(event.target.value)); }} onClick={(event) => event.stopPropagation()} className="h-1 w-0 accent-[#F3EBDD] opacity-0 transition-all group-hover/volume:w-20 group-hover/volume:opacity-100 focus-visible:w-20 focus-visible:opacity-100" aria-label="Preview volume" /><button onClick={(event) => { event.stopPropagation(); toggleMute(); }} className="grid h-8 w-8 place-items-center rounded-full bg-transparent text-[#F3EBDD] hover:bg-[#F3EBDD]/12" aria-label={muted ? 'Unmute preview' : 'Mute preview'}>{muted ? <VolumeX className="h-4 w-4 !text-[#F3EBDD]" /> : <Volume2 className="h-4 w-4 !text-[#F3EBDD]" />}</button></div><span className="absolute bottom-3 left-3 rounded-full bg-[#34483B]/85 px-2.5 py-1 text-[11px] font-semibold text-[#F3EBDD] shadow-lg backdrop-blur-md">{playing ? 'Previewing' : 'Preview'}</span></div>
