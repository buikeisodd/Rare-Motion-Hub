  ...
  return <div className="feed-shell min-h-screen bg-primary-background pb-24 text-primary-label md:pb-0">
    ...
    <main className="feed-main px-4 pb-20 pt-4 sm:px-8 lg:ml-28 xl:mr-80">
      <div className="mx-auto max-w-2xl py-3">
        ...
        <div className="h-36"><StoryRail stories={stories} user={user} onCreate={() => setCreateOpen(true)} onOpen={setStoryViewer} onDelete={async (id) => { const res = await fetch(`${apiUrl}/api/stories/${id}`, { method: 'DELETE' }); if (res.ok) setStories((current) => current.filter((story) => story.id !== id)); return res.ok; }} /></div>
