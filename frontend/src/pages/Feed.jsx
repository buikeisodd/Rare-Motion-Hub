...
function StoryViewer({ story, stories, onClose, onNavigate }) {
  ...
  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[#34483B]/75 p-3 backdrop-blur-2xl sm:p-6" onClick={onClose}>
    ...
    <div onClick={(event) => event.stopPropagation()} className="relative flex h-[min(88vh,720px)] w-[min(92vw,420px)] flex-col overflow-hidden rounded-[2rem] border border-[#F3EBDD]/30 bg-[#718A78] shadow-2xl">
      ...
      <div className="relative z-10 mt-7 flex items-center justify-between px-5 text-[#F3EBDD]"><span className="truncate text-sm font-semibold">{story.owner?.name || 'Story'}</span><button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-xl bg-[#34483B]/40" aria-label="Close"><X className="h-5 w-5" /></button></div>
