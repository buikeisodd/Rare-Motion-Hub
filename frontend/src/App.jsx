...
function WelcomeBack({ user, onDone }) {
  ...
  return (
    <div className="min-h-screen bg-primary-background text-[#34483B] px-6 py-12 flex flex-col relative overflow-hidden">
      <div className="pointer-events-none absolute -left-32 top-[-12rem] h-[28rem] w-[28rem] rounded-full bg-[#6F8974]/[0.14] blur-3xl" />
  ...
  return (
    <AnimatePresence mode="wait">
      <Routes location={location} key={location.pathname}>
        ...
        <Route path="/live/:id" element={<motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ duration: 0.3 }}><Live user={user} /></motion.div>} />
