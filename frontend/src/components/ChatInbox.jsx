...
function ConversationRow({ conversation, active, onClick }) {
  ...
  return <button onClick={onClick} className={`flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition-colors ${active ? 'bg-highlight' : 'hover:bg-highlight/60'}`}>
    ...
    <span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-primary-label">{name}</span><span className="flex min-w-0 items-center gap-2"><span className="min-w-0 flex-1 truncate text-xs text-secondary-label">{conversation.isRequest ? 'Message request' : conversation.lastMessage?.text || 'No messages yet'}</span><span className="shrink-0 text-[10px] text-secondary-label">{formatChatTime(conversation.lastMessage?.createdAt)}</span></span></span>
