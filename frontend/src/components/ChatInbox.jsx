...
function GroupSettings({ group, user, apiUrl, onBack, onUpdated }) {
  const [data, setData] = useState(group || {}); const [friends, setFriends] = useState([]); const [saving, setSaving] = useState(false); const [notice, setNotice] = useState('');
  ...
  return <div className="flex h-full min-w-0 flex-col"><header className="flex items-center gap-3 border-b border-border p-4"><button onClick={onBack} className="grid h-9 w-9 place-items-center rounded-xl bg-shading" aria-label="Back to group chat"><ArrowLeft className="h-4 w-4" /></button><h3 className="font-semibold">Group settings</h3></header><div className="flex-1 space-y-4 overflow-y-auto p-4"><div className="flex items-center gap-3"><div className="grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-full bg-highlight">{data.avatarUrl ? <img src={data.avatarUrl} alt="" className="h-full w-full object-cover" /> : <Users className="h-7 w-7" />}</div><div className="min-w-0 flex-1"><input disabled={!isAdmin && !data.membersCanEdit} value={data.name || ''} onChange={(event) => setData((current) => ({ ...current, name: event.target.value }))} onBlur={() => update({ name: data.name })} className="w-full bg-transparent text-lg font-semibold outline-none" /><input disabled={!isAdmin && !data.membersCanEdit} value={data.avatarUrl || ''} onChange={(event) => setData((current) => ({ ...current, avatarUrl: event.target.value }))} onBlur={() => update({ avatarUrl: data.avatarUrl })} placeholder="Group picture URL" className="mt-1 w-full rounded-lg bg-shading px-2 py-1 text-xs outline-none" /></div></div>{isAdmin && <div className="space-y-2 rounded-2xl bg-shading/60 p-3"><p className="text-xs font-semibold uppercase tracking-wider text-secondary-label">Admin controls</p><label className="flex items-center justify-between text-sm">Messaging open<input type="checkbox" checked={data.messagingOpen !== false} onChange={(event) => update({ messagingOpen: event.target.checked })} /></label><label className="flex items-center justify-between text-sm">Members can edit group<input type="checkbox" checked={Boolean(data.membersCanEdit)} onChange={(event) => update({ membersCanEdit: event.target.checked })} /></label><label className="flex items-center justify-between text-sm">Members can invite<input type="checkbox" checked={Boolean(data.membersCanInvite)} onChange={(event) => update({ membersCanInvite: event.target.checked })} /></label></div>}<div><h4 className="mb-2 text-sm font-semibold">Invite friends</h4><div className="space-y-2">{friends.filter((friend) => !(data.participantIds || []).includes(friend.id)).map((friend) => <div key={friend.id} className="flex items-center justify-between rounded-xl bg-shading/60 p-2 text-sm"><span className="truncate">{friend.name}</span><button disabled={saving} onClick={() => invite(friend)} className="rounded-lg bg-primary-label px-3 py-1 text-xs font-semibold text-primary-background">Invite</button></div>)}</div></div>{notice && <p className="text-sm text-secondary-label">{notice}</p>}</div></div>;
}
