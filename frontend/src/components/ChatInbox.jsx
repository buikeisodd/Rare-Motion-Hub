...
function ChatMenu({ user, privacy, setPrivacy, onBack, onProfile, onClear }) {
  return <><header className="flex items-center gap-3 border-b border-border p-4"><button onClick={onBack} className="grid h-9 w-9 place-items-center rounded-xl bg-shading" aria-label="Back to inbox"><ArrowLeft className="h-4 w-4" /></button><h3 className="font-semibold">Chat settings</h3></header><div className="flex-1 space-y-2 overflow-y-auto p-4"><Link to={`/profile/${user?.id}`} onClick={onProfile} className="flex items-center gap-3 rounded-2xl bg-shading p-4 hover:bg-highlight"><Settings2 className="h-5 w-5" /><span><strong className="block text-sm">Account</strong><small className="text-xs text-secondary-label">Open your profile</small></span></Link><div className="rounded-2xl bg-shading p-4"><div className="mb-3 flex items-center gap-3"><ShieldCheck className="h-5 w-5" /><strong className="text-sm">Privacy</strong></div>{[['lastSeen','Last seen'],['online','Online status'],['readReceipts','Read receipts']].map(([key,label]) => <label key={key} className="flex items-center justify-between border-t border-border py-3 text-sm"><span>{label}</span><input type="checkbox" checked={privacy[key]} onChange={() => setPrivacy((current) => ({ ...current, [key]: !current[key] }))} /></label>)}</div><button className="flex w-full items-center gap-3 rounded-2xl bg-shading p-4 text-left hover:bg-highlight"><Link2 className="h-5 w-5" /><span><strong className="block text-sm">Linked devices</strong><small className="text-xs text-secondary-label">Manage connected devices</small></span></button><button onClick={onClear} className="flex w-full items-center gap-3 rounded-2xl bg-shading p-4 text-left hover:bg-highlight"><Trash2 className="h-5 w-5" /><span><strong className="block text-sm">Storage management</strong><small className="text-xs text-secondary-label">Clear this chat</small></span></button><button className="flex w-full items-center gap-3 rounded-2xl bg-shading p-4 text-left hover:bg-highlight"><Star className="h-5 w-5" /><span><strong className="block text-sm">Starred messages</strong><small className="text-xs text-secondary-label">View saved messages</small></span></button></div></>;
}
...
export default function ChatInbox({ user, isOpen, onToggle, startConversationWith, onOpenStory }) {
  ...
  const openChatMenu = () => { setActive(null); setMessages([]); setReplyingTo(null); setMessageMenu(null); setChatMenuOpen(true); };
