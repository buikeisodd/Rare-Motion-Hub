...
function GroupSettings({ group, user, apiUrl, onBack, onUpdated }) {
  const [data, setData] = useState(group || {}); const [friends, setFriends] = useState([]); const [saving, setSaving] = useState(false); const [notice, setNotice] = useState(''); const [avatarFile, setAvatarFile] = useState(null); const [avatarPreview, setAvatarPreview] = useState('');
  ...
  const removeMember = async (memberId) => { if (!window.confirm("Remove this member from the group?")) return; const res = await fetch(`${apiUrl}/api/groups/${group.id}/members/${memberId}`, { method: "DELETE", credentials: "include" }); const value = await res.json().catch(() => ({})); if (!res.ok) return setNotice(value.error || "Could not remove member."); setData(value.group); onUpdated(value.group); };
  const deleteGroup = async () => { if (!window.confirm("Delete this group for everyone?")) return; const res = await fetch(`${apiUrl}/api/groups/${group.id}`, { method: "DELETE", credentials: "include" }); const value = await res.json().catch(() => ({})); if (!res.ok) return setNotice(value.error || "Could not delete group."); onBack(); };
